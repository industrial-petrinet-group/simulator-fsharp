...
module IntCSTests =
    ...
    [<Tests>]
    let tests =
        testList "Domain.ColorSets.IntCSTests." [
            testCase "Int color set can be created and it's value for '135' is 135" <| fun () ->
                printfn "%A" (Declaration.actuals())
                Ok <| Int 135 =! ColorSet.deserialize (CS "int") "135"
